...
export default function App() {
  ...
  // SYNC-015: heartbeat de la CAJA cada 30s a device_heartbeats (solo cuando
  // no estamos en modo monitor — el monitor no reporta su propia presencia).
  useHeartbeat(isMonitorMode ? null : deviceId);
        ...
        {tabs.map(tab => {
          ...
          return (
            <TabButton
              key={tab.id}
              id={tab.id}
              icon={<tab.icon size={isActive ? 20 : 18} strokeWidth={isActive ? 2.5 : 2} />}
              label={tab.label}
              isActive={isActive}
...
function TabButton({ id, icon, label, isActive, badgeCount, onClick, 'data-tour': dataTour }) {
  return (
    ...
    >
      <div className={`relative flex items-center justify-center w-10 h-10 sm:w-11 sm:h-11 rounded-full transition-all duration-300 shrink-0 ${
        isActive 
          ? 'bg-gradient-to-tr from-amber-500 via-amber-400 to-yellow-400 text-slate-950 shadow-lg shadow-amber-500/40 ring-2 ring-amber-400/50 scale-105' 
      ...
      <span className={`w-full max-w-full truncate text-center text-[9px] xs:text-[10px] sm:text-xs tracking-tight xs:tracking-normal leading-tight transition-colors duration-200 ${
        isActive ? 'text-white font-bold' : 'text-slate-400 font-medium group-hover:text-slate-200'
      }`}>
        {label}
      </span>
    </button>
